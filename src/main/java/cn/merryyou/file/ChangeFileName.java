...
public class ChangeFileName {
    ...
    static String oldString = "[www.javazx.com]";//要被替换的字符串
    static String rmFileName = "《javazx.com》Java自学网_用户必读.txt";
    //    static String oldString_2 = "_十次方学习禁广告闲聊群863676367";
    static String dir = "/Volumes/Elements/视频/javazx/20天Java开发大型电商系统谷粒商城实战课程";//文件所在路径，所有文件的根目录，记得修改为你电脑上的文件所在路径
    ...
    private static File reName(File newDir, String fileName, File parentPath, File file) {
        ...
        if (fileName.contains(oldString)) {//文件名包含需要被替换的字符串
            newName = fileName.replace(oldString, newString);//新名字
            newDir = new File(parentPath + "/" + newName);//文件所在文件夹路径+新文件名
            file.renameTo(newDir);//重命名
            System.out.println("修改后：" + newDir);
//            file.delete();
        } else if (fileName.equalsIgnoreCase(rmFileName)) {
            newDir = new File(parentPath + "/" + fileName);
            newDir.delete();
        }
        return newDir;
    }
